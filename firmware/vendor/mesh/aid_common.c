...
#define SPEECH_DMA_CHANNEL_MIC		(AUDIO_DMA_CHANNEL_MIC)
STATIC_ASSERT(!((AUDIO_MESH_EN == 1) && (SPEECH_ENABLE == 1))); // if want to enable both, need to change SPEECH_DMA_CHANNEL_MIC to a different one which is not occupied, such as DMA5.
...
void aid_init()
{
	...
	audio_rx_dma_chain_init(SPEECH_DMA_CHANNEL_MIC,(u16*)&iso_in_buff,MIC_BUFFER_SIZE*2);
