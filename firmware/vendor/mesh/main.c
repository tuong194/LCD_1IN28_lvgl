...
{
	FLASH_ADDRESS_CONFIG;
#if (PINGPONG_OTA_DISABLE && (0 == FW_START_BY_LEGACY_BOOTLOADER_EN) && (0 == FW_START_BY_BOOTLOADER_EN))
	...
	dp_burn_fw_enable(); // enable before user init and after gpio init, in case IO will be changed or while 1 in user init.
#endif
...
#if USE_DP_TO_BURN_FW // 
    dp_burn_delay_for_bdt_active(deepRetWakeUp, 0);
#endif

    irq_enable();
	#if (LOG_FW_FUNC_EN || DEBUG_LOG_SETTING_DEVELOP_MODE_EN || (MESH_USER_DEFINE_MODE == MESH_IRONMAN_MENLO_ENABLE))
