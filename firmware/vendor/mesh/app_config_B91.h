...
#define PCBA_B91_SEL			PCBA_C1T216A20_V1_2
	#else
...
#define FIRMWARE_SIZE_EXCEED_512K_EN		1
...
#elif MI_API_ENABLE
#define FLASH_PLUS_ENABLE     	0
#elif FIRMWARE_SIZE_EXCEED_512K_EN
...
#define PINGPONG_OTA_DISABLE    1 	// must
	#else
...
#if FLASH_PLUS_ENABLE
#define FW_START_BY_BOOTLOADER_EN       	1			// must 1 for B91m
#else
#define FW_START_BY_BOOTLOADER_EN       	0
#endif
#if FW_START_BY_BOOTLOADER_EN
	#if PINGPONG_OTA_DISABLE // for current mesh SDK with 2M flash map
// address setting please refer to FW_START_BY_BOOTLOADER_ADDR     	   			// ADDR_FLASH_PRODUCT_IMG
//#define OTA_PROGRAM_BOOTADDR_USER              		// ADDR_FLASH_OTA_IMG
	#else // for telink_bootloader SDK.
#define HASH_SHA256_EN						(1)			// 0 means hash with AES-MMO("Matyas-Meyer-Oseas", use AES128, similar to CBC, but not CBC), 1 means sha256.
#define LITTLE_ENDIAN_KEY_SIGNATURE_EN		(0)			/* 0 means big endianness(both key and signature are big), 1 means little endianness.
															if change to 1, private key of tl_check_fw.bat must be change to little endianness.
															*/
#define FW_SIZE_MAX_USER_K     	            (320)       // max (508k - FW_START_BY_BOOTLOADER_ADDR) for eagle, please comfirm whether it is enough or not.
#define FW_START_BY_BOOTLOADER_ADDR     	(0x10000)   // ADDR_FLASH_PRODUCT_IMG
#define OTA_PROGRAM_BOOTADDR_USER           (0x90000)   // ADDR_FLASH_OTA_IMG
#define FW_OFFSET_MAX_K     	            (FW_SIZE_MAX_USER_K + (FW_START_BY_BOOTLOADER_ADDR / 1024))
#define CHECK_FW_SIGNATURE_VALID_IN_BOOTLOADER_EN	1	// must be 1
	#endif
#endif

#if (MESH_IRONMAN_AP2T31F80_EN || (PCBA_B91_SEL == PCBA_C1T216A20_V1_2))
#define	USE_DP_TO_BURN_FW		1 // use DP pin to burn firmware, no SWS for this PCBA.
#endif
...
#if HCI_LOG_FW_EN
	#if (MESH_IRONMAN_AP2T31F80_EN || (PCBA_B91_SEL == PCBA_C1T216A20_V1_2))
#define DEBUG_INFO_TX_PIN           		UART1_TX_PE0 // the same as secure bootloader. // 
	#else
