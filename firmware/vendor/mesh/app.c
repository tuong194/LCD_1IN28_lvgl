...
_attribute_no_inline_ void user_init() // must add no inline, or it will be inline to main_() which is ramcode.
{
    ...
    ota_firmware_max_size = 0; // to cancel calling bls_ota_clearNewFwDataArea() inside blc_ota_initOtaServer_module()
	blc_ota_initOtaServer_module();
	bootloader_ota_setNewFirmwwareStorageAddress();
#else
	blc_ota_initOtaServer_module(); // will call bls_ota_clearNewFwDataArea_() inside.
#endif
