...
#else
#define BLT_RX_FIFO_CNT			16	// some phones may not support DLE, so use the same count with no DLE.
	#if DU_LPN_EN
#define BLT_TX_FIFO_CNT			32// low down the rentention cost part .
	#else
#define BLT_TX_FIFO_CNT         (MESH_BLE_NOTIFY_FIFO_EN ? 32 : 32) // TODO: can not set >32. set to 128 in extend mode, because there is no blt_notify_fifo_
	#endif
#endif
...
#define SCAN_PRICHN_RXFIFO_NUM			64	// due to too many nodes.
#else
#define SCAN_PRICHN_RXFIFO_NUM			16   //must be 2^n
#endif
