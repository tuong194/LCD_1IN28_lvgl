...
_attribute_ble_data_retention_	u16	flash_unlock_status; //record flash unlock status for different flash type
	...
	#if FLASH_PLUS_ENABLE // 2M flash
		#if FLASH_P25Q80U_SUPPORT_EN
#undef FLASH_P25Q80U_SUPPORT_EN
#define FLASH_P25Q80U_SUPPORT_EN				0 // skip 1M flash to save code size.
		#endif
	#endif
#endif
...
void app_flash_protection_ota_begin()
{
	if(flash_prot_op_cb){
		...
		u32 start_addr = FW_START_BY_BOOTLOADER_ADDR;
		#else
		u32 start_addr = 0;
		#endif

		flash_prot_op_cb(FLASH_OP_EVT_STACK_OTA_WRITE_NEW_FW_BEGIN, start_addr, ota_program_bootAddr + ota_firmware_size_k * 1024); // lock none due to both two firmware area need to be modify: new firmware and 4B of old fw.
	}
}
...
const flash_lock_area_support_t FLASH_LOCK_AREA_SUPPORT[] = { // area must be sorted from low to high.
	...
	#endif
	#if (FLASH_P25Q16SU_SUPPORT_EN) // 2M capacity
		#if FLASH_PLUS_ENABLE // 2M flash mapping
	{MID156085, {{FLASH_LOCK_LOW_64K_MID156085, (64/4)}, {FLASH_LOCK_LOW_1M_MID156085, (1024/4)}, 	// 64k for bootloader
			    {FLASH_LOCK_LOW_1792K_MID156085, (1792/4)}, {FLASH_LOCK_ALL_2M_MID156085, (2048/4)}}},		
		#else
	...
	#if (FLASH_P25Q32SU_SUPPORT_EN) // 4M capacity
		#if FLASH_PLUS_ENABLE // 2M flash mapping
	{MID166085, {{FLASH_LOCK_LOW_64K_MID166085, (64/4)}, {FLASH_LOCK_LOW_512K_MID166085, (512/4)},	// 64k for bootloader
			    {FLASH_LOCK_LOW_1M_MID166085, (1024/4)}, {FLASH_LOCK_ALL_4M_MID166085, (4096/4)}}},	// 1m for pingpong OTA with 508k firmware
		#else
...
u16 flash_change_address_to_flash_lock_reg(u32 lock_addr_end_in, int ceiling_flag, u32 *lock_addr_end_out)
{
	*lock_addr_end_out = 0;
	...
	foreach_arr(i, FLASH_LOCK_AREA_SUPPORT){ // TO get the flash lock area support in initial funtion should be better ?
