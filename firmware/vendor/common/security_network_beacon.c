...
int is_iv_index_invalid()
{
	...
	if(0 == memcmp(&invalid_iv, iv_idx_st.cur, sizeof(iv_idx_st.cur))){
		return TRUE;
	}
...
int iv_update_key_refresh_rx_handle(mesh_ctl_fri_update_flag_t *p_ivi_flag, u8 *p_iv_idx)
{
	...
	u8 iv_idx[4];
	p_flag[0]= *p_ivi_flag;
	memcpy(iv_idx,p_iv_idx,4);

	iv_update_key_refresh_rx_handle_cb(p_ivi_flag, p_iv_idx);
    ...
    if(iv_idx_st.searching_flag){
        ...
        if(mesh_ivi_greater_or_equal(iv_idx, iv_idx_st.cur, 1)){
            if(p_flag->IVUpdate){
                if(mesh_ivi_equal(iv_idx, iv_idx_st.cur, 1) && (!iv_idx_st.update_proc_flag)){
            ...
            if(IV_UPDATE_NORMAL == iv_idx_st.update_proc_flag){
                ...
                if(p_flag->IVUpdate){
                    ...
                    mesh_iv_update_enter_update2normal(); // will set iv_idx_st.searching_flag = 0 inside
                }
            }else if(IV_UPDATE_STEP2 == iv_idx_st.update_proc_flag){
                if(p_flag->IVUpdate){
                    // skip
    ...
    // no else, because need to handle in both normal mode or searching mode.
    {
    	if(IV_UPDATE_NORMAL == iv_idx_st.update_proc_flag){	// normal mode
    		if(mesh_ivi_greater_or_equal(iv_idx, iv_idx_st.cur, 1+1)){		// have been sure "<= i + 42" before.
...
void mesh_iv_update_start_check()
{
    ...
    || ((IV_UPDATE_STEP2 == iv_idx_st.update_proc_flag)&&(!is_enough_time_keep_state()))){
        ...
        if(!is_busy_segment_or_reliable_flow()){	// no need, wait to confirm
			...
			mesh_ivi_event_cb(iv_idx_st.searching_flag);
			LOG_MSG_LIB(TL_LOG_IV_UPDATE,(u8 *)&iv_idx_st, sizeof(iv_idx_st),"IV index enter step1, sno:0x%06x, iv:",mesh_adv_tx_cmd_sno);
