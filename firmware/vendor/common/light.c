...
_USER_CAN_REDEFINE_ void show_ota_result(int result)
{
	if(result == OTA_REBOOT_NO_LED){
		...
		blt_ota_writeBootMarkForBootloader();
#endif
    ...
    if(OTA_SUCCESS == result){
        start_reboot(); // not reboot is also ok, because not_set_boot_mark_flag is 1.
    }
#else
	// TODO: for B85m mesh, start up flag has been set start up flag before enter this func. so need to add not_set_boot_mark_flag like B91m.
#endif
}
