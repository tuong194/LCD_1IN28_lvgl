...
#if (FLASH_PLUS_ENABLE && (0 == __TLSR_RISCV_EN__))
 ...
 * @note        
 */
int factory_reset() // only for B85m 1M bytes flash, not for B91m.
{
	u32 r = irq_disable ();
	...
	for(int i = 0; i < (FLASH_ADR_AREA_1_END - FLASH_ADR_AREA_1_START) / 4096; ++i){
	    ...
	    if(adr != FLASH_ADR_RESET_CNT){ // will be optimized if FLASH_ADR_RESET_CNT is not between start and end address.
 ...
 * @note        
 */
int factory_reset(){ // for B85m 512 kbytes flash, for B91m 1M flash and 2M flash
	u32 r = irq_disable ();
#if (APP_FLASH_PROTECTION_ENABLE)
	u32 backup_addr = app_flash_protection_mesh_par_modify_much_sectors_begin(FLASH_ADR_AREA_1_START, FLASH_SIZE_MAX_SW);
#endif
