...
#if MESH_RX_TEST
    model_sig_cfg_s.relay_retransmit.val = model_sig_cfg_s.nw_transmit.val;
#endif
	mesh_model_store_cfg_s();
	#endif
...
#else
	#if VIRTUAL_ADDR_ENABLE
    u16 list[SUB_LIST_CNT_MAX + VIRTUAL_ADDR_CNT_MAX];
    #else
    u16 list[SUB_LIST_CNT_MAX];
    #endif
    foreach(i, SUB_LIST_CNT_MAX){
