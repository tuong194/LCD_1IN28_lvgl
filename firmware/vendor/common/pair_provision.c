...
void pair_prov_set_scan_mac(u8 *mac)
{
	...
	LOG_PAIR_PROV_DEBUG(mac, 6, "pair proc scan mac: ", 0);
}
...
void get_random_n(u8 *out, int len)
{
	...
	while(len > 0){
		...
		rand_val = rand();
		sleep_us(20);
		memcpy(out, ((u8 *)&rand_val) + index, cnt);
		index += cnt;
		len -= cnt;
	}
}
...
{
	LOG_PAIR_PROV_DEBUG(0, 0, "pair proc start", 0);
	if(0 == is_provision_success()){
		LOG_PAIR_PROV_DEBUG(0, 0, "pair proc create new network", 0);
		// ------- set net info -----------
...
void pair_proc_set_start_scan()
{
	...
	if(PAIR_PROV_PROC_ST_GET_ONLINE_NODES == p_proc->st){
		...
		if(is_provision_success()){
			...
			LOG_PAIR_PROV_DEBUG(0, 0, "pair proc reset to default network", 0);
		}else{
			LOG_PAIR_PROV_DEBUG(0, 0, "pair proc start error", 0);
			return ;
		}
	...
	if(PAIR_PROV_PROC_ST_SCAN == p_proc->st){
		if((MESH_ADV_TYPE_BEACON == p_beacon->header.type) && (UNPROVISION_BEACON == p_beacon->beacon_type)){
			...
			if(0 == memcmp(PAIR_PROV_UUID_HEAD_FLAG, p_beacon->device_uuid, sizeof(head_flag))){
				...
				LOG_PAIR_PROV_DEBUG((u8 *)p_beacon, p_beacon->header.len + 1, "rx unprov beacon:", 0);
...
{
	int err = 0;
	node_reset_start(0); // delay reboot for self aslo.
	return err;
}
	...
	switch(p_proc->st){
		...
		case PAIR_PROV_PROC_ST_SCAN:
			...
			if(clock_time_exceed(p_proc->tick, PAIR_PROV_SCAN_TIMEOUT_MS*1000)){
				LOG_PAIR_PROV_DEBUG(0, 0, "pair proc scan timeout", 0);
		...
		case PAIR_PROV_PROC_ST_CLOSE:
			if(p_proc->revert_network_flag){
				LOG_PAIR_PROV_DEBUG(0, 0, "pair proc reboot to back to normal network: %d", 0);
				start_reboot(); // TBD
			}
