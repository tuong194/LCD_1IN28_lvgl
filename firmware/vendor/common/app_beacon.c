...
int mesh_tx_sec_nw_beacon(mesh_net_key_t *p_nk_base, u8 blt_sts)
{
    ...
    if(blt_sts){
		...
		#endif
		if(0 == err){	
			LOG_MSG_LIB(TL_LOG_IV_UPDATE,(&bc_bear.len), bc_bear.len+1,"tx GATT secure NW beacon:"); // will trigger once in mesh_key_retrieve_() when BLE_MULTIPLE_CONNECTION_ENABLE
		}
	}else{
