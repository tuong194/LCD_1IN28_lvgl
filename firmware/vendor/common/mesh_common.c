...
#else
STATIC_ASSERT(FLASH_ADR_AREA_FIRMWARE_END <= (((MCU_CORE_TYPE == MCU_CORE_9518)||(MCU_CORE_TYPE == MCU_CORE_8278)) ? 0x7F000 : 0x3F000)); // assert for Pingpong OTA.
#endif
...
void mesh_ota_reboot_proc()
{
    ...
    if(ota_reboot_later_tick && clock_time_exceed(ota_reboot_later_tick, reboot_standby_ms*1000)){ // for 4 hops or more
        ...
        if(OTA_SUCCESS_DEBUG == ota_reboot_type){
            while(1){show_ota_result(OTA_SUCCESS_DEBUG);} // can not use OTA_SUCCESS, because write boot mark in it
        }else{
            if(OTA_SUCCESS == ota_reboot_type){
                ...
                if((0 == err) && (ADDITIONAL_NODE_UNPROVISIONED == fw_update_srv_proc.additional_info)){
                	...
                	factory_reset();// can not use kick_out(), because need to run write boot mark later by calling show_ota_result_()
                }
                #endif
            }
...
void bootloader_ota_setNewFirmwwareStorageAddress()
{
    ...
    ota_firmware_max_size = FW_SIZE_MAX_USER_K * 1024; // not include firmware offset
    blt_ota_reset();// to set "blotaSvr.last_adr_index = ota_firmware_max_size/16;" // should not extern blotaSvr, because they may be diffrent between diffrent SDKs.

    bls_ota_clearNewFwDataArea(0);

	#if 1 // CHECK_FW_SIGNATURE_VALID_IN_BOOTLOADER_EN
	extern u8 not_set_boot_mark_flag;
	not_set_boot_mark_flag = 1; // for to use blt_ota_writeBootMarkWithBootloader_()
	#endif
}
#endif
