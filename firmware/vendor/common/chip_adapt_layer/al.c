...
STATIC_ASSERT(USE_DP_TO_BURN_FW == 1);
#endif
...
#if (FW_START_BY_BOOTLOADER_EN && (0 == CHECK_FW_SIGNATURE_VALID_IN_BOOTLOADER_EN))
int blt_ota_writeBootMarkForBootloader(void)
{
#if 1 // in case invalid trigger in loop. // (BLE_SRC_TELINK_MESH_EN)
	//LOG_MSG_LIB(TL_LOG_NODE_BASIC,0,0,"writeBootMark, ota_program_offset: 0x%x", ota_program_offset);
    if(0 == is_valid_startup_flag(ota_program_offset + BOOT_MARK_ADDR, 0)){
		//LOG_MSG_LIB(TL_LOG_NODE_BASIC,0,0,"invalid startup flag");
	...
	LOG_MSG_LIB(TL_LOG_NODE_BASIC,0,0,"writeBootMark: 0x%x", ota_program_offset + BOOT_MARK_ADDR);
	//flag = 0;
	//flash_write_page((ota_program_offset ? 0 : ota_program_bootAddr) + BOOT_MARK_ADDR, 1, (u8 *)&flag);	//Invalid flag
	//LOG_MSG_LIB(TL_LOG_NODE_BASIC,0,0,"clearBootMark: 0x%x", (ota_program_offset ? 0 : ota_program_bootAddr) + BOOT_MARK_ADDR);
...
{
#if (FW_START_BY_BOOTLOADER_EN && (0 == CHECK_FW_SIGNATURE_VALID_IN_BOOTLOADER_EN))
	return 0; // blt_ota_writeBootMarkForBootloader(); // write boot mark in show_ota_result_(). if not, it will be write twice.
#else
...
{
	blt_ota_setResult(OTA_STEP_FEEDBACK, reset_flag);
}
...
_attribute_ram_code_ u8 mesh_blc_rx_aux_adv(u8 * raw_pkt, u8 * new_pkt)
{
	...
	if(!is_crc_ok(raw_pkt)){  // add by weixiong, receive abnormal adv, confirm later
		return 0;
	}

	rf_pkt_ext_adv_t *pExtAdv = (rf_pkt_ext_adv_t *)raw_pkt;
	int flag_aux_payload_with_src_sno = 0;
	if(pExtAdv->rf_len == pExtAdv->ext_hdr_len + 1 + sizeof(aux_payload_t)){
	...
	if(pExtAdv->ext_hdr_flg & EXTHD_BIT_AUX_PTR){
		...
		channel = p_curAux->chn_index;
		time_offset = (p_curAux->offset_unit?(sys_tick_per_us*300):(sys_tick_per_us*30)) * p_curAux->aux_offset;
		if(time_offset > (max_aux_offset_ms * 1000 * sys_tick_per_us)){ // may delay brx window in gatt connecting.
			return 0;
		} 
		
		extHdr_offset += EXTHD_LEN_3_AUX_PTR;
	}
#if 0 // save ram, because not use extHdr_offset any more. if enable, it will also be optimized by eclipse.
	/* 6. Sync info */
    ...
    *ph = 0;
    ...
    if (*ph)
    {
        ...
        int pkt_keep_flag = mesh_blc_aux_adv_filter(raw_pkt);
        ...
        if(RF_BLE_PACKET_VALIDITY_CHECK(raw_pkt)){ //CRC OK
			...
			int head_len = 2; // 2: ext_hdr_len	+ adv_mode + ext_hdr_flg;
			/* 1. advA: C1/C4, can not exist on both ADV_EXT_IND and AUX_ADV_IND */
			if(pExtAdv->ext_hdr_flg & EXTHD_BIT_ADVA){
				memcpy(tmp_mac, pExtAdv->data, 6);
				head_len += EXTHD_LEN_6_ADVA;
			}
			
			/* 2. targetA: 	C1/C2, at lease one exist, error if no one exist */
			if(pExtAdv->ext_hdr_flg & EXTHD_BIT_TARGETA){
				head_len += EXTHD_LEN_6_TARGETA;
			}

			/* 3. CTE info can not exist in "ADV_EXT_IND", jump */
			/* 4. ADI */
			if(pExtAdv->ext_hdr_flg & EXTHD_BIT_ADI){
				head_len += EXTHD_LEN_2_ADI;
			}

			/* 5. Aux Ptr */
			if(pExtAdv->ext_hdr_flg & EXTHD_BIT_AUX_PTR){
				head_len += EXTHD_LEN_3_AUX_PTR;
			}

			/* 6. Sync info */
			if(pExtAdv->ext_hdr_flg & EXTHD_BIT_SYNC_INFO){
				head_len += EXTHD_LEN_18_SYNC_INFO;
			}
			
			/* 7. Tx Power */
			if(pExtAdv->ext_hdr_flg & EXTHD_BIT_TX_POWER){
				head_len += EXTHD_LEN_1_TX_POWER;
			}
			
			// convert to legacy adv format rf_packet_adv_t before report.
			#define ADV_LEN_MAX 	(255)
			u8 rf_len = raw_pkt[DMA_RFRX_OFFSET_RFLEN];
  			if((rf_len>= 6+2) && (rf_len <= ADV_LEN_MAX) && (rf_len > head_len) && (raw_pkt[DMA_RFRX_OFFSET_RFLEN+1+10]<=(ADV_LEN_MAX - 7))){
				u8 copy_len = rf_len - head_len + 10; // 10: 3 crc + 4 time stamp + 2 freq + 1 rssi.
 				if(head_len >= 6){
					memcpy(pAdv->advA, tmp_mac, 6);
					memcpy(pAdv->data, raw_pkt + DMA_RFRX_OFFSET_DATA + head_len, copy_len); 
				}
				else{
					u8 right_shift = 6 - head_len;
					foreach(i, copy_len){
						raw_pkt[DMA_RFRX_OFFSET_RSSI(raw_pkt) + right_shift - i] = raw_pkt[DMA_RFRX_OFFSET_RSSI(raw_pkt) - i];
					}

					memcpy(pAdv->advA, tmp_mac, 6);
				}

				raw_pkt[DMA_RFRX_OFFSET_RFLEN] = rf_len + 6 - head_len;
			}
            // AUX
            next_buffer = 1;
            raw_pkt[2] = 0xAA;
...
#if 0// extend adv report in blt_leg_scan_mainloop_task()
...
ble_sts_t blc_ll_setScanEnable (scan_en_t scan_enable, dupFilter_en_t filter_duplicate)
{
	...
	if(scan_enable){
		...
		if((BLS_LINK_STATE_CONN == blc_ll_getCurrentState()) && (BLE_STATE_PRICHN_SCAN_S == get_ble_state())){
...
{
	return adv_filter_proc(raw_pkt, blc_ll_getCurrentState());
}
...
{
	if((BLS_LINK_STATE_CONN == blc_ll_getCurrentState()) && (BLE_STATE_BRX_S != get_ble_state()) && rf_link_time_allow((mesh_adv_extend_module==NULL) ? 3000 : 5000)){ // generic off 1.6ms, max payload is 1.8ms
...
#if (USE_DP_TO_BURN_FW)
/**
 * @brief       This function enable DP pin to burn firmware and debug in BDT tool.
 * @return      none
 * @note        
 */
/*_attribute_ahead_ram_code_ */void dp_burn_fw_enable()
{
...
#if 0
/**
 * @brief       This function disable DP pin to burn firmware and debug in BDT tool.
 * @return      none
 * @note        
 */
void dp_burn_fw_disable()				// only boot loader primary can lock flash, other firmware will keep the value or unlock.
{
    reg_swire_ctrl1 &= ~FLD_SWIRE_USB_EN; 	// eagle 0x80100c01[7]
    reg_usb_mdev &= ~FLD_USB_BDT_EN;		// disable accessing register through USB mode. default value is also 0 for kite.
    gpio_set_func(GPIO_DP, AS_GPIO);
}
#endif

/**
 * @brief       This function delay for bdt active
 * @param[in]   deepRetWakeUp	- flag of wake up from deep retention.
 * @param[in]   disable_dp_flag	- disable dp burning function to decrease power consumption for B91 both MCU working or suspend.
 * @return      must delay for BDT active
 * @note        
 */
void dp_burn_delay_for_bdt_active(bool4 deepRetWakeUp, bool4 disable_dp_flag)
{
	if (PM_DEEPSLEEP_RETENTION_ENABLE || disable_dp_flag){
		if(0 == deepRetWakeUp){
			foreach(i, 3){			// must delay for BDT active
				wd_clear(); 		// clear watch dog
				sleep_ms(1000);
			}
		}
		
		gpio_set_func(GPIO_DP, AS_GPIO); // or gpio_input_dis(GPIO_DP) // or dp_burn_fw_disable(); to decrease power consumption about 1.5mA for B91 when MCU is working.
	}
}
#endif
