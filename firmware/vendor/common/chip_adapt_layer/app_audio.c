...
//	analog_write_reg8 (0x02, 0xc4);		//flash 2.8V trim   codec ?
	...
	{
		audio_tx_dma_chain_init (AUDIO_DMA_CHANNEL_SPEAKER, (u16*)speaker_buff, speaker_size);
	}

	if (mic_buff && mic_size)
	{
		audio_rx_dma_chain_init(AUDIO_DMA_CHANNEL_MIC,(u16*)mic_buff, mic_size);
	}
}
...
static inline u32 tcodec_get_speaker_rptr (void) {
	return ((audio_get_tx_dma_rptr(AUDIO_DMA_CHANNEL_SPEAKER) - (u32)buff_playback) / sizeof (tcodec_int));
}

static inline u32 tcodec_get_mic_wptr (void) {
	return ((audio_get_rx_dma_wptr(AUDIO_DMA_CHANNEL_MIC) - (u32)buff_mic) / sizeof (tadc_int));
}
