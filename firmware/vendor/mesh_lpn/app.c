...
int soft_timer_test0(void)
{
	...
	LOG_MSG_LIB(TL_LOG_NODE_SDK,0, 0,"soft_timer_test:%d", soft_timer_cnt0);
	return 0;
}
#endif
...
void user_init_peripheral(int retention_flag)
{
	...
	if(BLS_LINK_STATE_ADV == get_blt_state()){
...
_attribute_ram_code_ void user_init_deepRetn(void)
{
	...
	if((BLS_LINK_STATE_ADV == get_blt_state()) && is_friend_ship_link_ok_lpn() && (!my_fifo_get(&mesh_adv_cmd_fifo)) && ( (0 == fri_ship_proc_lpn.poll_tick) || clock_time_exceed(fri_ship_proc_lpn.poll_tick, get_lpn_poll_interval_ms()*1000/2)) &&
		get_blt_busy() && !mesh_lpn_subsc_pending.op){ // not soft timer wakeup
